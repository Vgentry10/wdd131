...
function search() {
  const recipeQuery = searchInput.value.trim().toLowerCase();
  ...
  const filteredRecipes = recipes.filter((recipe) => {
    ...
    return (
      name.includes(recipeQuery) ||
      description.includes(recipeQuery) ||
      tags.some((tag) => tag.toLowerCase().includes(recipeQuery))
    );
  });

  const sortedRecipes = filteredRecipes.sort((a, b) => (b.rating || 0) - (a.rating || 0));
...
function ratingTemplate(rating = 0) {
  ...
  for (let i = 1; i <= 5; i++) {
    html += i <= Math.round(rating)
      ...
      : `<span aria-hidden="true" class="star-empty">☆</span>`;
  }
  html += `</span>`;
  return html;
}
...
function recipeCardTemplate(recipe) {
  const imageSrc = recipe.image || recipe.imageUrl || recipe.img || '';
  const genre = Array.isArray(recipe.tags) && recipe.tags.length ? recipe.tags[0] : 'Recipe';

  return `
    <section class="recipe-card">
      <div class="recipe-details">
        <img src="${imageSrc}" alt="${recipe.name}" class="recipe-image">
...
function renderRecipes(list) {
  if (!Array.isArray(list) || list.length === 0) {
    mainContainer.innerHTML = `<section class="recipe-card"><div class="recipe-details"><p class="description">No recipes found.</p></div></section>`;
    return;
  }

  mainContainer.innerHTML = list.map(recipeCardTemplate).join('');
}
